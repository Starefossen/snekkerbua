...
eneste kilden: **hver tegning, hver tabell og alle 32 sidene i byggeheftet og
alle 60 i referanseheftet genereres av solidene og maskinsjekkes før de får
finnes.** Ikke ett mål er skrevet av. Unntaket er fire skjemaark i `docs/schematics/`,
...
Bokstavene, fyllkoden, antallene i beslaglegenden under bildet og halvsnittet
er alle utledet — og en assert som feller bygget måler det ferdige blekket og beviser at
...
| **Trevirke** | **85 stykker** i **5 dimensjoner** pluss én 18 mm kryssfinerplate — 54,65 løpemeter. Fordelingen: 23×98 34 stk. · 48×68 20 stk. · 36×48 16 stk. · 36×98 12 stk. · 48×98 2 stk. · plata 1 stk. 24 av de 85 stykkene er ett og samme stykke: spilen, 23×98 × 800 mm, kappet i én innstilling. Tallene telles av modellen og står nederst i [kapplista](docs/generated/kappliste.md) — `TOTAL 85 pcs 54.65 m in 5 timber profiles + 1 sheet` er det modellen skriver ut mens den bygger |
| **Stål** | **203 festemidler fordelt på 26 ledd**, **194 av dem modellert som solide kropper** — hode, forsenking, skaft og spiss, hver med sin egen drivvektor. De 9 som mangler er de to veggfestene, J14 og J12-V: de går rett inn i veggen og har ingen andre del å ende i, så de er plassert, men ikke modellert. **Ikke ett eneste hode står i en romvendt flate**, og det er en assert. Begge tallene skriver modellen ut selv — `203 festemidler plassert i 26 ledd` og `194 festemidler modellert som kropper` |
| **Kontroller** | **512 asserter i modellen** og 237 til i verktøyene, alle sammen stopper bygget — tallene er talt, ikke anslått, og de telles som `ast.Assert`-noder i syntakstreet, ikke med grep: en grep på linjer som begynner med ordet «assert» tar med brødtekst som gjør det samme, og gir 519 her. Metoden er `python -c "import ast,sys;print(sum(isinstance(n,ast.Assert) for n in ast.walk(ast.parse(open(sys.argv[1]).read()))))" generate_loftbed.py`, og den samme summen over `tools/*.py` (24 filer) — men ingen runde arver dem lenger: `tools/check_tall.py` teller dem på nytt i porten og feller bygget hvis denne tabellen sier noe annet enn det den finner. Skrueretningene er utledet av fysikk (7 av de 24 radene i [skrueretningene](docs/generated/skrueretninger.md) er tvunget av tykkelsene alene), og hver av dem har en plasseringslinje som sier hvor på delen hullet står — 25 linjer over 24 skrueretninger og alle 194 festemidlene, en bijeksjon som asserteres på det ferdige blekket. De to siste radene er veggfestene: de får hver sin egen plasseringslinje over 9 fester, satt etter stender og ikke etter mål, og telles derfor for seg; antall skruer må få plass på flaten de står på; hver del må røre resten av sengen og kollidere med ingenting |
| **Determinisme** | `mise run check` kjører hele kjeden to ganger og krever **142 byte-identiske artefakter** — de tre filmene inkludert, pluss et hash-stempel som feller porten hvis en film er eldre enn modellen den viser. 142 er ikke et anslag: det er `git ls-files` over nøyaktig de stiene `snap()` i `mise.toml` hasjer. Determinismen er en assert, ikke en forventning |
| **Ut av det** | **To trykkeklare PDF-er** med én kommando — et byggehefte på **32 sider** og et referansehefte på **60**, `pdfinfo docs/hanna.pdf` og `pdfinfo docs/hanna-referanse.pdf` sier hvor mange det ble. Byggeheftet er det som tas med til benken; referanseheftet blir stående i hylla. Pluss en ren billedmanual, en skrevet byggeveiledning, sju skjemategninger, to boresjablonger i 1:1, to bruksark og eksport til STEP / STL / GLB / USDZ |
...
generate_loftbed.py           modellen: mål, deler, festemidler, asserter
  ...
  ├─ tools/render_boresjablong.py → docs/schematics/boresjablong-*.svg (1:1)
...
* **Plasseringsmålene på stegarkene.** Hvert steg skylder de få målene som
  plasserer treet det legger til — avstanden fra noe som allerede står til
  kanten av delen, utledet av kroppene og leddene og aldri satt for hånd. Hver
  pil måles tilbake ut av det ferdige `<path>`-et, gjennom kameraet, mot
  modellens eget tall; og en bijeksjon holder «tegnet på arket» mot «skyldt av
  steget», så et mål ikke kan forsvinne stille. Høyder måles nedover, aldri fra
  en fot og aldri fra gulvet — X6 regel 2, denne gangen på tegningen.
...
mise run pdf        # docs/hanna.pdf, 32 sider + docs/hanna-referanse.pdf, 60 sider
```
...
| `boresjablong` | Tegner de to 1:1-boresjablongene til `docs/schematics/boresjablong-*.svg` — arkene som skrives ut i full størrelse, klippes fra hverandre og legges på treet |
| `figurhode` | Regner hodet på konturfiguren inn i de fire figurikonene og skriver landemerketabellene i PRAKSIS §4 |
| `check` | Determinismeasserten: to fulle kjøringer, 142 artefakter, byte-identisk eller feil — og etterpå `check_tall` (README-ens talte tall + tallsveipet over håndprosaen) og `falsifiser` (feilinjisering av vokterne) |
| `pdf` | Setter sammen `docs/hanna.pdf` (byggeheftet) og `docs/hanna-referanse.pdf` av de innsjekkede dokumentene (trenger ikke build123d) |
...
| `docs/hanna.pdf` | Byggeheftet på 32 sider — stegsidene og det som trengs ved benken. Bevisst utenfor git; verktøyet ligger i repoet, og fila er én `mise run pdf` unna |
| `docs/hanna-referanse.pdf` | Referanseheftet på 60 sider — begrunnelsene, tallene og de øvrige tegningene. Samme sak: bygges, ikke sjekkes inn |
...
<img src="docs/img/hanna-manual-sider.png" alt="De ni første sidene i byggeheftet, tre og tre" width="760">

*De ni første av byggeheftets 32 sider, lest rett tilbake ut av `docs/hanna.pdf`
av `tools/render_pdf_matrix.py`: forsiden, innholdet, senga med sine seks mål,
de tre sidene om å måle rommet, de to sidene med konvensjoner og sikkerhet, og
beslagsiden. Hver eneste side på det arket er kompilert —
...
* `docs/hanna.pdf` — stegsidene og resten av byggeheftet, satt opp for trykk,
  og `docs/hanna-referanse.pdf` — begrunnelsene og tallene. Ikke innsjekket;
  kjør `mise run pdf`, så kommer de, identiske, ut av de innsjekkede
  dokumentene.
...
/ OpenCascade. The model is the only source of truth: every drawing, every
table, all 32 pages of the printed build booklet and all 60 of its reference
companion are generated from the solids and machine-checked before they are
allowed to exist, and no number is hand-transcribed. Four schematic sheets in `docs/schematics/` are the one
...
`mise run check` builds the whole chain twice and demands 142 byte-identical
