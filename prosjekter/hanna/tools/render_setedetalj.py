...
from schematic import Sheet, Frame, f, nb, esc, sizes   # noqa: E402


# ---------------------------------------------------------------------------
# TYPE AND STROKE - the family's numbers, scaled to this sheet
# ---------------------------------------------------------------------------
# They live in tools/schematic.py, because the hand-drawn sheets in
# docs/schematics/ and this one have to agree about them - and two files that
# must agree about a number is one file too many.
SZ = sizes(STYLE_K)
...
def field3(sh, G, SB, SS):
    ...
    out = []
    for row in rows:
        out += sh.wrap(row, COL_4 - MARG - 40.0)
...
def field4(sh, G, SB):
    ...
    out = []
    for row in rows:
        out += sh.wrap(row, SHEET_W - MARG - COL_4)
    ty = oy + T + 52.0
...
def build(G):
    sh = Sheet(SHEET_W, SHEET_H, STYLE_K,
              "Loftseng - skraaskruesetet og vinkelklossen")
